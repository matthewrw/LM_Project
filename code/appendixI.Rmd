...
m2 = lm(sqISI ~temp + wind + summer + rainvnorain, weights = weight
        , data = train)
m3 = lm(sqISI ~ temp + wind + summer + rainvnorain+tFFMC
        , data = train)
df = data.frame(
  r1 = as.vector(qqnorm(resid(m1), plot=F))
...
+ ggtitle("No FFMC")+theme_bw() + coord_cartesian(ylim = c(-3,3))
